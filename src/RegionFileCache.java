 ...
 * 2011 March 2
 * 
 * CAUTION: Modified for beta 1.3 by chris@hackernet.co.uk
 * Unmodified version available from http://mojang.com/2011/02/16/minecraft-save-file-format-in-beta-1-3/
 * Thanks to Mojang and Scaevolus for the original code.
 * 
 */
...
public class RegionFileCache {
	...
	public static synchronized RegionFile getRegionFile(File basePath, int chunkX, int chunkZ) {
		...
		return getRegionFileInner(file);
	}

	public static synchronized RegionFile getRegionFile(File basePath, String fileName) {
		File regionDir = new File(basePath, "region");
		File file = new File(regionDir, fileName);

		return getRegionFileInner(file);
	}

	private static synchronized RegionFile getRegionFileInner(File file) {
